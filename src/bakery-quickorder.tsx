...
import { Search, ShoppingCart, Copy, Share2, X, Instagram, Phone, ChevronDown, Check, Minus, Plus } from 'lucide-react';
...
const CONFIG = {
  ...
  logo: "🧁",
  heroImage: "https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=1200&h=600&fit=crop"
};
...
const MENU_DATA = {
  ...
  items: [
    ...
    },
    {
      ...
      price: "₹280",
      weight: "6 pcs",
      image: "https://images.unsplash.com/photo-1587668773472-3aa380f5ca6e?w=400&h=300&fit=crop"
    },
    {
      id: 7,
    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div className="p-4">
        ...
        <QuantityControl
          ...
          onDecrease={() => onQuantityChange(item.id, Math.max(0, quantity - 1))}
        />
      </div>
    </div>
  );
    ...
    <header className="bg-white shadow-sm sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="text-3xl" role="img" aria-label="logo">{CONFIG.logo}</span>
          <h1 className="text-xl font-bold text-gray-800">{CONFIG.bakeryName}</h1>
        </div>
...
const OrderPreview = ({ order, customerInfo, onCustomerInfoChange, onCopyOrder, onWhatsAppShare, onClose, isOpen }) => {
  const orderItems = Object.entries(order).filter(([_, data]) => data.quantity > 0);
  ...
  return (
    <>
      {/* Mobile bottom sheet */}
      <div className={`fixed inset-x-0 bottom-0 bg-white rounded-t-3xl shadow-2xl z-50 transition-transform duration-300 lg:hidden ${isOpen ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="p-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <ShoppingCart className="w-5 h-5 text-pink-600" />
              <h3 className="font-bold text-lg">Your Order ({orderItems.length})</h3>
            </div>
            <button onClick={onClose} className="p-2" aria-label="Close order preview">
              <X className="w-5 h-5" />
            </button>
          </div>
          <div className="max-h-60 overflow-y-auto mb-4 space-y-3">
            {orderItems.map(([id, data]) => (
              <div key={id} className="flex justify-between items-start text-sm border-b pb-2">
                <div className="flex-1">
                  <p className="font-semibold">{data.item.name}</p>
                  <p className="text-gray-600">Qty: {data.quantity} × {data.item.price}</p>
                  {data.notes && <p className="text-gray-500 text-xs mt-1">Note: {data.notes}</p>}
                </div>
              </div>
            ))}
          </div>
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <button
                ...
                title={!hasItems ? "Select items to copy order" : "Copy order to clipboard"}
              >
          ...
          {orderItems.length === 0 ? (
            ...
            orderItems.map(([id, data]) => (
              <div key={id} className="border-b pb-3">
                ...
                <p className="text-gray-600 text-sm">Qty: {data.quantity} × {data.item.price}</p>
                <input
                  type="text"
                  placeholder="Add note (optional)"
                  value={data.notes}
                  onChange={(e) => onCustomerInfoChange('note', e.target.value, id)}
...
export default function BakeryQuickOrder() {
  ...
  const orderItemCount = Object.values(order).reduce((sum, data) => sum + data.quantity, 0);
